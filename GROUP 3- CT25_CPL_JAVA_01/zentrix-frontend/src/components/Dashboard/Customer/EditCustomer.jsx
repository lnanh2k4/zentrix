...
import { editCustomer } from "@/context/ApiContext";
...
const EditCustomer = ({ isOpen, onClose, onSubmit, customer }) => {
    ...
    // Khởi tạo dữ liệu khách hàng khi component mở
    useEffect(() => {
        if (customer) {
            setCustomerData({
                ...
                dob: customer.dob ? new Date(customer.dob) : null, // Chuyển thành Date object
                phone: customer.phone || "",
                address: customer.address || "",
                sex: customer.sex !== undefined ? String(customer.sex) : "",
    ...
    const validateForm = () => {
        ...
        else if (!/^[0-9]{0,15}$/.test(customerData.phone)) newErrors.phone = "Phone must be a 15-digit number";
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            onSubmit((await editCustomer(payload)) || payload);
            setCustomerData(defaultCustomerData);
            setErrors({});
            showNotification("Update customer successfully", 3000, 'complete');
            onClose();
        } catch (error) {
            console.error("Error updating customer:", error.response?.data?.message, error.response?.data?.content);
            showNotification(
                `Failed to update customer: ${error.response?.data?.content || error.message}`,
                3000,
                "fail"
            );
        }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-xl">
                <h2 className="text-lg font-semibold text-left mb-6">Edit Customer</h2>

...
const EditBranch = ({ isOpen, onClose, onSubmit, branch }) => {
    // Khởi tạo state trực tiếp với branch nếu có
    ...
    // Cập nhật state khi branch thay đổi
    useEffect(() => {
        if (branch && JSON.stringify(branch) !== JSON.stringify(branchData)) {
            setBranchData({
                brchName: branch.brchName || "",
                address: branch.address || "",
                phone: branch.phone || "",
                status: branch.status !== undefined ? branch.status : 1,
            });
        }
    }, [branch]);
    ...
    const handleInputChange = (e) => {
        ...
        if (name === "phone") {
            ...
            setBranchData({ ...branchData, [name]: Number(value) });
        } else {
            setBranchData({ ...branchData, [name]: value });
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(
                `http://localhost:6789/api/v1/branches/${branch.brchId}`,
                payload,
                {
            ...
            if (error.response?.data?.code) {
                switch (error.response.data.code) {
                    case 1100:
                        ...
                        break;
                    case 1106:
                        errorMessage = "Branch name is already taken";
                        break;
                    case 1107:
                        errorMessage = "Branch name contains invalid special characters";
                        break;
                    case 1103:
                    case 1105:
            ...
            showNotification(errorMessage, 3000, "fail");
        } finally {
    ...
    const handleCancel = () => {
        // Reset về dữ liệu ban đầu khi cancel
        if (branch) {
            setBranchData({
                brchName: branch.brchName || "",
                address: branch.address || "",
